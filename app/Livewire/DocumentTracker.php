...
class DocumentTracker extends Component
{
    ...
    public function fetchTracker() {
        ...
        $this->logs = Logs::where('userID', Auth::user()->id)->where('documentID', $qrcodeID)->get();
        $this->returnedlogs = ReturnedLogs::where(['userID' => Auth::user()->id])->where('documentID', $qrcodeID)->get();
    }
    /**
     * Handle an incoming real time updates.
     *
